...
void parser_core_header32(struct core_data_t* core_data, Elf32_Ehdr *ehdr) {
...
void parser_write_core_header32(struct core_data_t* core_data, Elf32_Ehdr *ehdr) {
    fwrite(ehdr, sizeof(Elf32_Ehdr), 1, core_data->fp);
}

void parser_core_note32(struct core_data_t* core_data, Elf32_Phdr *note) {
    note->p_type = PT_NOTE;
    note->p_offset = sizeof(Elf32_Ehdr) + core_data->phnum * sizeof(Elf32_Phdr);
}

void parser_write_core_note32(struct core_data_t* core_data, Elf32_Phdr *note) {
...
void parser_core_prstatus32(struct core_data_t* core_data) {
...
void parser_core_auxv32(struct core_data_t* core_data) {
    core_data->auxvnum = PARSER_SIZE(mm_struct_saved_auxv) / sizeof(Elf32_auxv);

    core_data->auxv_cache = malloc(core_data->auxvnum * sizeof(Elf32_auxv));
...
void parser_core_load_vma32(struct core_data_t* core_data, int index) {
    Elf32_Phdr* phdr = &((Elf32_Phdr* )core_data->load_cache)[index];
    memset(phdr, 0x0, sizeof(Elf32_Phdr));

    phdr->p_type = PT_LOAD;
    phdr->p_vaddr = (Elf32_Addr)core_data->vma_cache[index].vm_start;
    phdr->p_paddr = 0x0; 
    phdr->p_memsz = (Elf32_Addr)core_data->vma_cache[index].vm_end
                    - (Elf32_Addr)core_data->vma_cache[index].vm_start;
...
void parser_write_core_program_headers32(struct core_data_t* core_data, Elf32_Phdr *note) {
    if (!core_data->vma_count) return;

    Elf32_Phdr* prev;
    parser_core_load_vma32(core_data, 0);
    Elf32_Phdr* phdr = &((Elf32_Phdr*)core_data->load_cache)[0];
    phdr->p_offset = align_up(note->p_offset + note->p_filesz, core_data->align_size);
    fwrite(phdr, sizeof(Elf32_Phdr), 1, core_data->fp);

    for (int index = 1; index < core_data->vma_count; ++index) {
        prev = &((Elf32_Phdr*)core_data->load_cache)[index - 1];
        phdr = &((Elf32_Phdr*)core_data->load_cache)[index];
        parser_core_load_vma32(core_data, index);
...
void parser_write_core_auxv32(struct core_data_t* core_data) {
    ...
    while (index < core_data->auxvnum) {
        fwrite((char *)core_data->auxv_cache + index * sizeof(Elf32_auxv), sizeof(Elf32_auxv), 1, core_data->fp);
        index++;
    }
}
...
void parser_write_core_file32(struct core_data_t* core_data, Elf32_Phdr *note) {
    ...
    uint32_t number = core_data->vma_count;
    fwrite(&number, 4, 1, core_data->fp);
    uint32_t page_size = core_data->page_size;
    fwrite(&page_size, 4, 1, core_data->fp);
    ...
    while(index < core_data->vma_count) {
        ...
        fwrite(&ntfile, sizeof(Elf32_ntfile), 1, core_data->fp);
        index++;
    }
...
void parser_core_note_align32(struct core_data_t* core_data, Elf32_Phdr *note) {
    ...
    uint32_t offset = align_up(note->p_offset + align_filesz, core_data->align_size);
    uint32_t size = offset - (note->p_offset + align_filesz);
    fwrite(core_data->zero_buf, size, 1, core_data->fp);
}

void parser_write_core_load32(struct core_data_t* core_data) {
    physaddr_t paddr;
    uint32_t vaddr;
    int idx, i;
    ...
    for (idx = 0; idx < core_data->vma_count; ++idx) {
        Elf32_Phdr* phdr = &((Elf32_Phdr*)core_data->load_cache)[idx];
        ...
        for (i = 0; i < count; ++i) {
            ...
            if (paddr) {
                if (page_exist) {
                    readmem(paddr, PHYSADDR, core_data->page_buf, core_data->page_size, "write load32", QUIET);
...
void parser_core_dump32(struct core_data_t* core_data) {
    ...
    core_data->load_cache = malloc(core_data->vma_count * sizeof(Elf32_Phdr));
    core_data->phnum = core_data->vma_count + 1;
    core_data->fill_vma_name(core_data);
    parser_core_header32(core_data, &ehdr);
    parser_core_note32(core_data, &note);
    parser_core_prstatus32(core_data);
    parser_core_auxv32(core_data);

    parser_write_core_header32(core_data, &ehdr);
    parser_write_core_note32(core_data, &note);
    parser_write_core_program_headers32(core_data, &note);
    core_data->parser_write_core_prstatus(core_data);
    parser_write_core_auxv32(core_data);
    parser_write_core_file32(core_data, &note);
    parser_core_note_align32(core_data, &note);
    parser_write_core_load32(core_data);
